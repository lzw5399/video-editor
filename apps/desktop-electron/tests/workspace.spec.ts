...
test("text edit routes complete text inspector changes through project session intent observations", async () => {
  ...
  try {
    ...
    for (const [section, locator] of [
      ["文本", textSection],
      ["样式", styleSection],
      ["文本框", textBoxSection],
      ["布局", layoutSection]
    ] as const) {
      await expect(locator.getByRole("heading", { name: section, exact: true })).toBeVisible();
    }
...
test("字幕 SRT import intent path sends raw SRT once without renderer-created cue segments", async () => {
  ...
  try {
    ...
    expect(
      ...
      "SRT subtitle editing must not let renderer choose text source"
    ).toBeNull();
    expect(editTextCall?.textPatch, "SRT subtitle editing must use a text patch").not.toBeNull();
    expect(editTextCall?.textContent).toBe("第一句字幕 已校对");
    const visualCall = calls.findLast((call) => call.command === "updateSelectedSegmentVisual");
    expect(visualCall?.visual ?? null, "SRT subtitle visual editing must not send full SegmentVisual").toBeNull();
    expect(
      visualCall?.visualPatch?.positionX,
...
test("音频预览 panel and inspector expose production audio controls through updateSelectedSegmentAudio intent", async () => {
  ...
  try {
    ...
    await expect(audioPanel.getByRole("button", { name: "应用音频" })).toHaveCount(0);
...
test("画面变换 command-only transform 通过 Rust command 更新 UI 并清理派生状态", async () => {
  ...
  try {
    ...
    const visualCall = (await readNativeCommandObservations(app)).findLast((call) => call.command === "updateSelectedSegmentVisual");
    expect(visualCall?.kind).toBe("updateSelectedSegmentVisual");
    expect(visualCall?.visual ?? null, "workspace visual form must not send full SegmentVisual").toBeNull();
    expect(visualCall?.visualPatch).toMatchObject({
      visible: true,
      fitMode: "fill",
      backgroundKind: "black",
      positionX: 160,
