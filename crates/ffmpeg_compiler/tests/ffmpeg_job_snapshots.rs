...
fn filters_snapshot_uses_stable_labels_for_video_audio_text_outputs() {
    ...
    assert_eq!(
        ...
        [
            ...
            "[vstage0a]null[v0]",
            "[1:v]trim=start=0.600000:duration=0.100000,setpts=PTS-STARTPTS,scale=1920:1080[vstage1a]",
            "[vstage1a]null[v1]",
            "color=c=black:s=1920x1080:r=30/1:d=0.100000[vbase0]",
            "[vbase0][v0]overlay=x=0:y=0:shortest=1[vbase1]",
            "[vbase1][v1]overlay=x=0:y=0:shortest=1[vbase2]",
...
#[test]
fn audio_filters_preserve_target_timeline_placement_before_mixing() {
    let job = compile_ffmpeg_job(
        &common::export_plan_with_delayed_audio_mix(),
        &common::compile_context(),
    )
