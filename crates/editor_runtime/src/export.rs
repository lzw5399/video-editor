...
use draft_model::{
    Draft, ExportDiagnostic, ExportDiagnosticKind, ExportJobPhase, ExportJobStatusResponse,
    ExportPrepDirtyFacts, ExportPreset, ExportValidationReport, Microseconds,
...
use media_runtime::{
    CancelToken, FfmpegExecutor, FfmpegJobEvent, FfmpegJobResult, FfmpegJobState,
    FfmpegRuntimeError, FfmpegRuntimeJob, OutputValidationError, OutputValidationExpectation,
    RuntimeCapabilityReport, RuntimeConfig, validate_rendered_output,
};
...
pub struct ExportService {
    next_id: u64,
    scheduler: SchedulerExportService,
}
...
impl ExportService {
    ...
    pub fn start_export(
        ...
        self.scheduler.start_export(runtime, payload)
    }
    ...
    {
        self.scheduler
            .start_export_with_validation_executor(runtime, payload, validation_executor)
    }
...
impl SchedulerExportService {
    ...
    fn telemetry_snapshot(&self) -> SchedulerTelemetrySnapshot {
        ...
        state.scheduler.telemetry_snapshot()
    }
...
pub fn global_export_registry() -> &'static ExportService {
    static REGISTRY: OnceLock<ExportService> = OnceLock::new();
    REGISTRY.get_or_init(ExportService::default)
}
