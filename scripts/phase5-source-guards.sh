...
rg -n "ffmpegArgs|filterComplex|ffmpegScripts|renderGraph|exportScript|processHandle|validationExpectation|filter_complex" \
  apps/desktop-electron/src/renderer \
  | rg -v "renderGraphFailed" && {
    echo "renderer must not own export scripts, render graph, process handles, or validation expectations" >&2
    exit 1
  } || true
